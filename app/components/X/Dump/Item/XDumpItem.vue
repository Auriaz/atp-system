...
<template>
    <div class="py-0.5">
        <div class="flex items-center space-x-2 group" :class="{ 'cursor-pointer hover:bg-basic-800/20': expandable }" @click="expandable && (open = !open)">
            <Icon 
                v-if="expandable"
            ...
            <span v-if="!expandable" class="pl-2">
                ...
                <span v-else-if="type === 'boolean'" class="text-info-400">{{ value ? 'true' : 'false' }}</span>
        ...
        >
            <div v-if="expandable && open">
                <x-dump :value="value" :name="`${index}`" :is-nested="true" />
            </div>
        </transition>
    </div>
</template> 