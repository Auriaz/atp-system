...
const confirmAction = ref({
  ...
  description: '',
  confirmText: '',
  color: 'error' as 'error' | 'warning' | 'success',
...
// Lifecycle - wait for authentication before fetching
onMounted(async () => {
  ...
  if (isAuthenticated.value) {
    ...
    const stopWatcher = watch(isAuthenticated, async (authenticated) => {
      if (authenticated) {
        await fetchSessions()
        stopWatcher() // Stop watching once we've fetched
      }
    })
  }
})
...
<template>
  <div class="space-y-6">
    ...
    <div v-if="!isAuthenticated" class="bg-white dark:bg-gray-800 rounded-lg p-6 shadow-sm">
      <div class="flex items-center justify-center py-8">
        <UIcon name="i-heroicons-arrow-path" class="w-6 h-6 animate-spin text-gray-400" />
        <span class="ml-2 text-gray-500">Sprawdzanie autoryzacji...</span>
      </div>
    </div>

    <!-- Normal session management content when authenticated -->
    <template v-else>
